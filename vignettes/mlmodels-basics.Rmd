...
The only model that does not accept a scale argument is `ml_poisson()`, because its variance is fully determined by the mean.
...
- **[Exploring the variance types](mlmodels-variance.html)** — How to get different types of variances (`oim`, `robust`, `boot`, `jack`, clustered), when to use each, and how to store a bootstrapped or jackknifed variance matrix for reuse.
...
- **Gamma versus lognormal** — Comparing predictions and variance behavior on income data.
