...
* You can also **pre-compute** any of our variance matrices (including bootstrapped and jackknifed), using `vcov()`, and pass the resulting matrix through the `vcov` argument. This is especially recommended for bootstrapped and jackknifed standard errors when you plan to compute multiple predictions or marginal effects.
* **`predictions()`** also offers **additional uncertainty methods** — such as full bootstrap, simulation (Krinsky-Robb), `rsample`, etc. — which you can control more through its `inferences()` function.
...
plot_slopes(fit_gamma, 
            variables = "minors",
            condition = "age",
            vcov = "robust") +
  labs(title = "Marginal Effect of Having Children on Family Income",
