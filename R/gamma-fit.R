                     ...
                     ...)
{
  ...
  suppressMessages({
    ml0 <- .ml_gamma.fit(y = y, x = x0, z = z0, w = w_scaled)
  })
  
  # Get the unweighted individual log-likelihood contributions for the null model
  ll0_vec <- .ml_negbin_ll(b = ml0$estimate, 
                           y = y, 
                           x = x0, 
                           z = z0, 
                           w = rep(1, length(y)))   # force weights = 1
  
  model_list$ll0 <- sum(as.numeric(ll0_vec))
