...
#'
#' @param constraints Character vector of constraints (e.g. c("value::wt = 0",
#'   "value::income = 2 * value::price", "scale::wt >= 0.01")) or NULL.
#' @param coef_names Character vector of all coefficient names in the model
#'   (e.g. c("value::(Intercept)", "value::wt", "scale::lnsigma", ...))
#'
#' @return A list with `eqA`, `eqB`, `ineqA`, `ineqB` suitable for maxLik,
#'   or NULL if no constraints were provided.
#'
#' @keywords internal
parse_constraints <- function(constraints = NULL, coef_names)
{
  ...
  if (!is.character(constraints)) {
    cli::cli_abort("`constraints` must be a character vector of string constraints.",
                   call = NULL)
  }
  ...
  for (constr in constraints) {
    ...
    if (grepl(">=|<=|=", constr)) {
      ...
      # Safely convert RHS to numeric with nice error message
      rhs <- tryCatch({
        as.numeric(rhs_str)
      ...
      if (is.na(rhs)) {
        cli::cli_abort(
          c("Invalid constraint: Right-hand side must be a number.",
            "i" = "Constraint: {.val {constr}}",
            "x" = "Found {.val {rhs_str}} on the right-hand side, which is not numeric."),
          call = NULL
        )
      }

      # Parse left-hand side into coefficients
      row <- parse_linear_expression(lhs, coef_names)

      if (op == "=") {
        eq_rows[[length(eq_rows) + 1]] <- row
        eq_rows[[length(eq_rows)]] <- c(eq_rows[[length(eq_rows)]], -rhs)
      } else if (op == ">=") {
        ineq_rows[[length(ineq_rows) + 1]] <- row
        ineq_rows[[length(ineq_rows)]] <- c(ineq_rows[[length(ineq_rows)]], -rhs)
      } else if (op == "<=") {
        ineq_rows[[length(ineq_rows) + 1]] <- -row
        ineq_rows[[length(ineq_rows)]] <- c(ineq_rows[[length(ineq_rows)]], rhs)
      }
    } else {
      cli::cli_abort("Constraint must contain =, >= or <= : {.val {constr}}",
                     call = NULL)
    }
  }
  ...
  if (length(eq_rows) > 0) {
    eqA <- do.call(rbind, lapply(eq_rows, function(r) r[-length(r)]))
    eqB <- sapply(eq_rows, function(r) r[length(r)])
    result$eqA <- eqA
    result$eqB <- eqB
  }

  if (length(ineq_rows) > 0) {
    ineqA <- do.call(rbind, lapply(ineq_rows, function(r) r[-length(r)]))
    ineqB <- sapply(ineq_rows, function(r) r[length(r)])
    result$ineqA <- ineqA
    result$ineqB <- ineqB
  }
